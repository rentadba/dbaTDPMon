...
sqlcmd.exe -S%server% %autentif% -i "..\patches\20180628-patch-upgrade-from-v2018_5-to-v2018_6-hc.sql" -d %dbname%  -b -r 1
if errorlevel 1 goto install_err
...
:done
if "%run2kmode%"=="false" sqlcmd.exe -S%server% %autentif% -Q "SET NOCOUNT ON; UPDATE [dbo].[appConfigurations] SET [value] = N'2018.06.28' WHERE [module] = 'common' AND [name] = 'Application Version'" -d %dbname%  -b -r 1
if errorlevel 1 goto install_err  
