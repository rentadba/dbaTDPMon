RAISERROR('Create view : [dbo].[vw_statsHealthCheckDatabaseDetails]', 10, 1) WITH NOWAIT
GO
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vw_statsHealthCheckDatabaseDetails]'))
DROP VIEW [dbo].[vw_statsHealthCheckDatabaseDetails]
GO

CREATE VIEW [dbo].[vw_statsHealthCheckDatabaseDetails]
/* WITH ENCRYPTION */
AS
...
INNER JOIN [dbo].[statsHealthCheckDatabaseDetails] shcdd ON shcdd.[catalog_database_id] = cdn.[id] AND shcdd.[instance_id] = cdn.[instance_id]
GO
