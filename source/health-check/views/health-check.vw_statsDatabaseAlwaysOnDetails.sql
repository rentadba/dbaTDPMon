...
SELECT 	  cin.[project_id]		AS [project_id]
		...
		, cdn.[active]			AS [instance_active]
		, cdn.[state]			AS [database_state]
		, CASE WHEN cin.[active] = 0 THEN 'INSTANCE OFFLINE' ELSE cdn.[state_desc] END AS [database_state_desc]
		, sdaod.[cluster_name]
		, sdaod.[ag_name]
		, sdaod.[role_desc]
		, sdaod.[replica_join_state_desc]
		, sdaod.[replica_connected_state_desc]
		, sdaod.[failover_mode_desc]
		, sdaod.[availability_mode_desc]
		, sdaod.[synchronization_health_desc]
		, sdaod.[synchronization_state_desc]
		, sdaod.[suspend_reason_desc]
