...
-- Description		 : Generate Reports: Daily Health Check
-------------------------------------------------------------------------------
-- Change date		 : 
-- Description		 : 
-------------------------------------------------------------------------------
...
SET @job_name = @databaseName + N' - Generate Reports'
...
BEGIN TRANSACTION
	...
	---------------------------------------------------------------------------------------------------
	SET @queryToRun = N'EXEC [dbo].[usp_sqlAgentJobStartAndWatch]	@sqlServerName			= @@SERVERNAME,
						@jobName			= ''dbaTDPMon - Discovery & Health Check'',
						@stepToStart			= 1,
						@stepToStop			= 99,
	...
	EXEC @ReturnCode = msdb.dbo.sp_add_jobstep	@job_id=@jobId, 
												@step_name=N'Run Discovery and Health Check', 
												@step_id=1, 
	...
	SET @queryToRun = N'EXEC  [dbo].[usp_reportHTMLBuildHealthCheck]	@projectCode		= ''' + @projectCode + ''',
							@analysisJobName		= ''dbaTDPMon - Discovery & Health Check'',
							@flgActions		= DEFAULT,	
							@flgOptions		= DEFAULT,
